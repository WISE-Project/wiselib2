...
def GaussNoise(N,Sigma):
    ...
    y = Gauss1d(N,Sigma)
    y = fft.fftshift(y)
    r = 2*pi * random.rand(N)
    f = fft.ifft(y * exp(1j*r))
    return real(f)
'''
...
'''
    ...
    #     Eval
    #================================================
    def Eval(self, z = array(None) , y = array(None)):
...
#        Normalization = self.L * self.(Alpha)/(Lambda * )
        #R = array((sqrt((xa - xbi)**2 + (ya - ybi)**2)))
...
#     FUN: HuygensIntegral_1d
#==============================================================================
def ComputeSampling(Lambda, z, L0, L1,  Theta0, Theta1, OversamplingFactor = 1 ):
    '''
    Lambda: wavalenght
...
#                Det_ds = sqrt((Det_x[0] - Det_x[-1])**2 + (Det_y[0] - Det_y[-1])**2)
#                # E1
