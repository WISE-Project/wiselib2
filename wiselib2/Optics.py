...
#		DeltaX = self.SmallDisplacements.Long * np.cos(self.RayInNominal.Angle + DeltaAngle)
#		DeltaY = self.SmallDisplacements.Long * np.sin(self.RayInNominal.Angle + DeltaAngle)
...
#		self._XProp_Centre = XProp_Centre
		...
		self._f1 = np.norm(self._XYProp_Centre - self._XYProp_F1)
		self._f2 = np.norm(self._XYProp_Centre - self._XYProp_F2)
...
#		self._c = 0.5 * np.sqrt(f1**2 + f2**2 - 2*f1*f2*cos(pi - 2*Alpha))
		self._b = np.sqrt(self.a**2 - self.c**2)

		# computed 2 (aux)
		elle = 2*self._c
		_TmpArg = tl.Coerce(self._f2/elle * np.sin(pi - 2*Alpha),-1,1)
...
#		elle = 2*self._c
##		self._ThetaProp = np.arcsin(self._f2/elle * np.sin(pi - 2*self.Alpha))
...
##		self.Alpha = self._ThetaProp_to_Alpha(self._ThetaProp)
	...
	def _UpdateParametersLab(self):
		...
		self.VersorLab = UnitVector(Angle =  self._pTanLab_Angle - pi/2 - self.AngleGrazingNominal  )
	...
	def _UpdateFociiFromParameters_XMid(self, XMid, DeltaX):
		...
		self._f1 = np.norm(self.XYCentre - self.XYF1)
		#self._f1 = np.sqrt((self.XYCentre[0] - self.XYF1[0])**2 + (self.XYCentre[1] - self.XYF1[1])**2)
		self._f2 = np.norm(self.XYCentre - self.XYF2)
	...
	def _EvalMirrorXProp(self, YProp, Sign = +1):
		...
		'''
		# remove points which do not belong to ellipse domain.
		YProp = np.delete(YProp, [(YProp < -self.b) | (YProp > self.b)]) 
...
#		yOut = yOut if IsArray else yOut[0]
	...
	def GetXY_IdealMirror (self, N, Sign = +1, ReferenceFrame = 'lab', L = None):
		...
		#-----------------------------------------------------------------
		if L is None: # the stored length self.L is used, hence the stored XYStart
...
#		self._c = 0.5 * np.sqrt(np.cos(Alpha)**2 * (f1+f2)**2 + np.sin(f1-f2)**2)
		self._c = 0.5 * np.sqrt(f1**2 + f2**2 - 2*f1*f2*np.cos(pi - 2*Alpha))
		self._b = np.sqrt(self.a**2 - self.c**2)

		# computed 2 (aux)
		elle = 2*self._c
		TmpArg = tl.Coerce(self._f2/elle * np.sin(pi - 2*Alpha),-1,1)
		self._ThetaProp = np.arcsin(TmpArg)
		self._XYProp_F1 = [-self.c, 0]
		self._XYProp_F2 = [self.c,0]
		_XProp_Centre = f1*np.cos(self._ThetaProp) + self._XYProp_F1[0]
...
#		# SIAMO SICURI? non manca un np.arctan?!?!?!
#		return abs(Alpha - abs(m ))
		tmpArg = tl.Coerce(self._f2/(2*self._c) * np.sin(pi - 2*self._Alpha),-1,1)
		self._ThetaProp = np.arcsin(TmpArg)
		# @todo
